...
export class DashboardComponent implements OnInit {
	...
	readonly badges: (keyof typeof GymBadge)[];

	constructor(private us: UserService, private toast: ToastrService) {
		...
		this.us
			...
			.subscribe({
				next: (userInfo) => {
					...
					this.count = this.badgeCounts.reduce(
						(acc, cur) => acc + cur,
						0
					);
				},
				error: (e: Error) => {
					this.loading = false;
					this.toast.error(
						`Could fetch user data because: ${e.message}`,
						'Dashboard Error'
					);
				}
			});
