...
export class MapComponent implements OnInit, OnDestroy {
	...
	private obv: NextObserver<PopupReturn | undefined>;
	...
	constructor(
		...
		this.obv = {
			next: (ret) => {
				...
				switch (ret.type) {
					case 'gymUpdateFailed':
						this.toast.error(
							`Couldn't update gym because: ${JSON.stringify(
								ret.data
							)}!`,
							'Gym',
							{ disableTimeOut: true }
					...
					case 'badgeUpdateFailed':
						this.toast.error(
							`Couldn't update gym badge because: ${
								ret.data as string
							}!`,
							'Gym',
							{ disableTimeOut: true }
					...
					case 'gymUpdate': {
						...
						const ix = this.gyms.features.findIndex(
							({ properties: { firestoreId: fid } }) =>
								fid === u.firestoreId
						);

						if (ix === -1) {
							this.toast.error(
								`Couldn't update gym "${u.name}" because it doesn't exist in this layer!`,
								'Gym',
								{ disableTimeOut: true }
							);
						} else {
			...
			error: (e: Error) => {
				this.toast.error(
					`Popup closed with error ${e.message}`,
					'Popup',
					{ disableTimeOut: true }
				);
			}
	...
	ngOnInit(): void {
		...
		this.map = new MapboxMap({
			...
			style: environment.MAPBOX_STYLE_URI,
			zoom: 13,
		...
		this.us
			...
			.subscribe((isAdmin) => {
				if (isAdmin)
					this.map.addControl(new NewGymControl(), 'top-right');
			});
	}
				...
				next: (msg) => {
					...
					switch (msg.type) {
						case 'newGym': {
							...
							this.userBadges[feature.properties.firestoreId] =
								data.badge;
							this.map.setLayoutProperty(
								'gymsLayer',
								'icon-image',
	...
	private onGymsClick(
		e: MapMouseEvent & EventData & { features?: MapboxGeoJSONFeature[] }
	) {
		if (!e.features) {
			return;
		}
		...
		this.db
			...
			.pipe(
				tap((gymCollection) => {
					...
					(this.map.getSource('gymSource') as GeoJSONSource).setData(
						this.gyms
					);
			...
			.subscribe({
				next: (frag) => {
					const feat = this.gyms.features.find(
	...
	private get gymIcon(): Expression {
		return [
			'concat', // concatenate the following two strings
